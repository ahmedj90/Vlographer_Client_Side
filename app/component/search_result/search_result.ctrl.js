...
angular.module('vlographerApp')
  .controller('search_resultCtrl',['$scope', 'search_resultFactory', 'channelFactory', '$routeParams', 'userFactory', 'DOMAIN','Page','$timeout', function($scope, search_resultFactory, channelFactory, $routeParams, userFactory, DOMAIN,Page, $timeout){    
       $scope.DOMAIN=DOMAIN;   
      Page.setTitle('Vlographer | ' + $routeParams.topic.replace(/_/g," ")); 
      ...
      $scope.busy=false;      
      $scope.channels =[];
      $scope.title= $routeParams.topic.replace(/_/g," ");
      //url received genre
      ...
      function getChannels(offset, limit)
      {
        ...
        params["topicId"]=$scope.topicId;
          
        search_resultFactory.getChannelsByTopic(params)
            .then(function success(responses) {
